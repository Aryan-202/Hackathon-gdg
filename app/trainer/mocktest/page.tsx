...
const MockTestPage = () => {
  ...
  const [currentQuestion, setCurrentQuestion] = useState(0); // Using currentQuestion consistently
  ...
  const handleNext = () => {
    if (selectedOption === MOCK_QUESTIONS[currentQuestion].answer) {
      setScore((prev) => prev + 1);
    }

    if (currentQuestion < MOCK_QUESTIONS.length - 1) {
      setCurrentQuestion((prev) => prev + 1); // Fixed variable here
      setSelectedOption(null);
      setTimeLeft(30);
    } else {
        ...
        <div className="w-full max-w-4xl">
          ...
          {gameState === "start" && (
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-10 flex flex-col items-start text-left shadow-2xl transition-all duration-300 hover:border-purple-500/40">
              ...
              <h2 className="text-3xl font-bold text-white mb-4">Placement Readiness Test</h2>
              <p className="text-gray-400 mb-8 max-w-2xl leading-relaxed">
                This test simulates a real technical screening. You have 30 seconds per question.
              </p>
              <button
                onClick={startQuiz}
              ...
              >
                Start Assessment <IconChevronRight className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
          )}
            ...
            <div className="w-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
              <div className="flex justify-between items-end mb-4">
                <div className="space-y-1">
                  <h2 className="text-xl font-semibold text-white">Your Assessment</h2>
                  <p className="text-sm text-gray-500">
                    Question <span className="text-purple-400 font-bold">{currentQuestion + 1}</span> of {MOCK_QUESTIONS.length}
                  </p>
                </div>
                <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-xl border border-white/10">
                  <IconClock size={20} className={timeLeft < 10 ? "text-red-500 animate-pulse" : "text-purple-400"} />
                  <span className={`text-lg font-mono font-bold ${timeLeft < 10 ? "text-red-500" : "text-white"}`}>
                    00:{timeLeft < 10 ? `0${timeLeft}` : timeLeft}
                  </span>
                </div>
              </div>

              <Progress value={((currentQuestion + 1) / MOCK_QUESTIONS.length) * 100} className="mb-10 h-1.5 bg-white/5" />
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {MOCK_QUESTIONS[currentQuestion].options.map((option) => (
                    <button
                      key={option}
                ...
                <div className="pt-4 flex justify-end">
                  ...
                  >
                    {currentQuestion === MOCK_QUESTIONS.length - 1 ? "Submit Assessment" : "Next Question"}
                  </button>
                </div>
              </div>
            </div>
          ...
          {gameState === "finished" && (
            <div className="max-w-3xl bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-10 flex flex-col items-start text-left shadow-2xl">
              ...
              <div className="flex items-center gap-12 mb-10 w-full bg-white/5 p-8 rounded-2xl border border-white/10">
                <div>
                  <div className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-1">Final Score</div>
                  <div className="text-5xl font-black text-purple-500">
                    {score}<span className="text-2xl text-gray-600">/{MOCK_QUESTIONS.length}</span>
                  </div>
                </div>
              </div>
